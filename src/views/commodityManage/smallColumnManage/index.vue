  ...
  <div class="wrap">
    ...
    <section class="panel">
      ...
      >
        ...
        <el-table-column label="专栏名称" width="400">
  ...
  methods: {
    ...
    async down(row) {
      ...
      if (status === "02") {
        ...
        // 判断详情数据
        const resDataList = res.data.childSingleGoodsList;
        // 有详情数据
        if (resDataList.length >= 1) {
          ...
          // 无数据
          this.$message.warning("请先维护专栏详情信息，再上架");
        }
      }
    },
    share() {
    ...
    async getList(params = this.searchData) {
      const data = await requestList(getSmallColumnList, params, this.page);
      console.log(data);
