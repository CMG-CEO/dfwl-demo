  ...
  <div class="wrap">
    ...
    <section class="panel">
      ...
      >
        ...
        <el-table-column label="大专栏名称" width="400">
    ...
    <Edit
      ...
      :type="CommodityManageType.BIG_COLUMN_MANAGE"
      @reload="getList()"
    ></Edit>
    <!-- 详情 -->
    <Detail
...
import {
  ...
  delBigColumn,
  getBigColumn,
  changeBigColumnStatus,
} from "@/api/merchandiseManagement/bigColumnManage";
  ...
  methods: {
    ...
    async down(row) {
      ...
      if (status === "02") {
        ...
        await changeBigColumnStatus({ shelfStatus, shelfType, goodsId });
        ...
        if (resDataList.length >= 1) {
          ...
          // 无数据
          this.$message.warning("请先维护大专栏详情信息，再上架");
        }
      }
    },
    share() {
    ...
    async getList(params = this.searchData) {
      const data = await requestList(getBigColumnList, params, this.page);
